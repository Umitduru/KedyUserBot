...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        level=DEBUG,
        format="[%(asctime)s - %(levelname)s] - @kedyuserbot : %(message)s",
        datefmt='%d-%b-%y %H:%M:%S')
else:
    basicConfig(
        level=INFO,
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/Umitduru03/KedyUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/Umitduru03/KedyUserBot.git" 
EMERGENCY = "https://github.com/Umitduru03/KedyUserBot.git" # Acil durrum için
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @kedyuserbot Paketi |")
...
with bot:
    ...
    try:
        ...
        bot(JoinChannelRequest("@kedyuserbot"))
        bot(JoinChannelRequest("@kedysupport"))
        bot(JoinChannelRequest("@kedyplugin"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @kedyuserbot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Kedy açabilirsin; Kanala bak` @kedyuserbot')
            else:
                await event.reply(f'`Tengri save Turks! Kedy working... `')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@kedyuserbot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**En Gelişmiş UserBot!** [Kedy](https://t.me/kedyuserbot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@kedyuserbot",
                    text="""@kedyuserbot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/kedyuserbot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/kedysupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Umitduru03/KedyUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @kedyuserbot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Kedy](https://t.me/kedyuserbot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
