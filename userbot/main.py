...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, KEDY_VERSION, PATTERNS, DEFAULT_NAME, BOT_TOKEN
...
ALIVE_MSG = [
    "        **Hey {kedysahip} **\n \n✨ Yüklenen Plugin Sayısı {plugin}\n \n👨🏼‍💻 Python Sürümü {python}\n \n⚡️Telethon Sürüm {telethon}\n \nEndişelenme Kedy UserBot'un Sapa Sağlam Çalışıyor iyi günler :)️ <3😼\n\n\n         Kedy Sürüm {kedy} ",
    "🎆 `Endişelenme! Seni yanlız bırakmam.` **{kedysahip}**, `KedyUserbot😼 çalışıyor.` \n Bot Versiyonu: {kedy} ",
    "`⛈️ Elimden gelenin en iyisini yapmaya hazırım`, **{kedysahib}** \n Bot Versiyonu: {kedy} ",
    "✨ `KedyUserBot sahibinin emirlerine hazır...` \n Bot Versiyonu: {kedy} ",
    "`Şuan en gelişmiş userbotun düzenlediği mesajı okuyor olmalısın` **{kedysahip}**. \n Bot Versiyonu: {kedy} ",
...
CV_MSG = [
    "**{DEFAULT_NAME}** `Fazla Bi Bilgi Ayarlamamış Ama Şunu Biliyorum Kendisi Baya Zevkli Birisi Çünkü Kedy Userbot Kullanıyor.` 😁",
    "`Üzgünüm sana vercek bir bilgim yok.`"
]


UNAPPROVED_MSG = ("`Hey olduğun yerde kal,!👨‍💻 Ben` @kedyuserbot . `Endişelenme!`\n\n"
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # KEDYPY
            Kedypy = re.search('\"\"\"KEDYPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Kedypy == None:
                Kedypy = Kedypy.group(0)
                for Satir in Kedypy.splitlines():
...
try:
    ...
    if idim in herlockbl:
        bot.send_message("me", f"`❌ Kedy yöneticileri sizi bottan yasakladı! Bot kapatılıyor...`")
        LOGS.error("Kedy yöneticileri sizi bottan yasakladı! Bot kapatılıyor...")
        bot.disconnect()
...
LOGS.info("+===========================================================+")
LOGS.info("|                     ✨Kedy Userbot✨                     |")
LOGS.info("+==============+==============+==============+==============+")
LOGS.info("|                                                           |")
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          "Yardıma İhtiyacınız varsa, Destek grubumuza gelin t.me/kedyuserbot")
LOGS.info(f"Bot versiyonunuz: Kedy ==> {KEDY_VERSION}")
